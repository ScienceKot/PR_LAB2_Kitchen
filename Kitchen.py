...
class Kitchen:
    def __init__(self, cooks, n_ovens, n_stoves, menu_path):
        for i in range(len(cooks)):
            ...
            cooks[i] = Cook(
                ...
                name=cook['name'],
                catch_phrase=cook['catch_phrase']
            )
        self.cooks = cooks
        self.n_ovens = n_ovens
        self.n_stoves = n_stoves
        self.menu = json.load(open(menu_path, 'r'))
        self.order_list = []
    ...
    def prepare_food(self, order):
        cooking_time = 0
        self.taken_orders.append(order)
        ...
        order["prepared_time"] = time.mktime(now.timetuple())*1e3
        self.prepared_foods.append(order["order_id"])
        self.taken_orders[-1] = order
        return order
